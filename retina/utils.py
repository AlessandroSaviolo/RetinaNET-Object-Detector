...
def decode_boxes_and_labels_on_image_array(
        image,
        boxes,
        classes,
        scores,
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        if scores is None or scores[i] > min_score_thresh:
            box = tuple([int(round(b)) for b in boxes[i].tolist()])
            ...
            if scores is None:
                ...
                if not skip_labels:
                    if not agnostic_mode:
                        ...
                        predicted_labels.append(int(display_str.strip('\'')) if display_str != '0' else 10)
                if not skip_scores:
                    if not display_str:
    ...
    return {'bbox': predicted_boxes, 'score': predicted_scores, 'label': predicted_labels}


def decode_output(image, boxes, labels, probs, class_labels, min_score_thresh):
    category_index = {}
    ...
    return decode_boxes_and_labels_on_image_array(image, boxes, labels, probs, category_index, min_score_thresh=min_score_thresh)
